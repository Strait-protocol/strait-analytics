...
const ROUTES = ["BTC_TO_HEMI", "HEMI_TO_BTC"] as const;

export default async function BtcTunnelPage({ searchParams }: { searchParams: PageSearchParams }) {
    ...
    <Box component="main" sx={{ flex: 1, py: { xs: 3, sm: 5 } }}>
      <Container maxWidth="lg">
        <Stack spacing={4}>
          ...
          {vm && <TunnelPageContent vm={vm} granularity={granularity} routes={ROUTES} />}
        </Stack>
      </Container>
    </Box>
  );
